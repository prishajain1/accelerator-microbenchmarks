...
xpk workload create \
  ...
  python src/run_benchmark.py --config=configs/xlml_v5p_128_utksharma.yaml \
  && gsutil -m cp -r /tmp/microbenchmarks gs://v5p-microbenchmarks/v5p-128-$(date +%Y-%m-%d-%H-%M-%S)/" \
